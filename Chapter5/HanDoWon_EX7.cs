using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class HanDoWon_EX7 : MonoBehaviour
{
    public int a;
    // Start is called before the first frame update
    void Start()
    {
        
        switch (a)
        {
            case 1:
                Debug.Log("1월은 : 31일 까지 있습니다.");
                break;
            case 2:
                Debug.Log("2월은 : 31일 까지 있습니다.");
                break;
            case 3:
                Debug.Log("3월은 : 31일 까지 있습니다.");
                break;
            case 4:
                Debug.Log("4월은 : 31일 까지 있습니다.");
                break;
            case 5:
                Debug.Log("5월은 : 31일 까지 있습니다.");
                break;
            case 6:
                Debug.Log("6월은 : 31일 까지 있습니다.");
                break;
            case 7:
                Debug.Log("7월은 : 31일 까지 있습니다.");
                break;
            case 8:
                Debug.Log("8월은 : 31일 까지 있습니다.");
                break;
            case 9:
                Debug.Log("9월은 : 31일 까지 있습니다.");
                break;
            case 10:
                Debug.Log("10월은 : 31일 까지 있습니다.");
                break;
            case 11:
                Debug.Log("11월은 : 31일 까지 있습니다.");
                break;
            case 12:
                Debug.Log("12월은 : 31일 까지 있습니다.");
                break;
            default:
                break;
        }

        // Update is called once per frame
        void Update()
    {
        
    }
    }
}